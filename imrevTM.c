...
long ts = *((int *) tid); ts *= inf.vpxs/NTHRDS;
long te = ts+inf.vpxs/NTHRDS-1;
for(row=ts; row<=te; row++){
memcpy((void *)buf, (void *)myimg[row], (size_t)
inf.hbytes);
col=0;
while(col<inf.hpxs*3/2){
px.b = buf[col];
...
char Revtype[50];
switch (argc){
case 3 : NTHRDS=0; Rev = 'h'; break;
case 4 : NTHRDS=0; Rev = tolower(argv[3][0]);
break;
case 5 : NTHRDS=atoi(argv[4]);
...
printf("\n\nExample: imrevTM pic.bmp out.bmp w 8\n\n");
printf("\n\nExample: imrevTM pic.bmp out.bmp v 0\n\n");
printf("\n\nNothing ... Exiting ...\n\n");
exit(EXIT_FAILURE);
}
...
switch(Rev){
case 'h' : ThRevfunc = ThRevImgH; Revfunc=RevImgH; strcpy(Revtype,"horizontal (h)"); break;
case 'v' : ThRevfunc = ThRevImgV; Revfunc=RevImgV; strcpy(Revtype,"vertical (v)"); break;
case 'i' : ThRevfunc = ThRevImgHM; Revfunc=RevImgH; strcpy(Revtype,"horizontal (i)"); break;
case 'w' : ThRevfunc = ThRevImgVM; Revfunc=RevImgV; strcpy(Revtype,"vertical (w)"); break;
default : printf("Rev option '%c' is invalid. Can only be 'h', 'v', 'i', or 'w'\n",Rev);
...
for(i=0; i<NTHRDS; i++) pthread_join(ThHdle[i], NULL);
}
else{
for(a=0; a<RNDS; a++) (*Revfunc)(myimg);
}
gettimeofday(&t, NULL);
end = (double)t.tv_sec*1000000.0 + ((double)t.tv_usec);
elaptime=(end-start)/1000.00; elaptime/=(double)RNDS;
bmp_write(myimg, argv[2]);
for(i = 0; i < inf.vpxs; i++) free(myimg[i]);
free(myimg);
printf("\n\nTotal execution time: %9.4f ms. ",elaptime);
if(NTHRDS>1) printf("(%9.4f ms per thread). ",elaptime/(double)NTHRDS);
printf("\n\nFlip Type = '%s'",Revtype);
