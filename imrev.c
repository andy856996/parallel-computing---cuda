...
int main(int argc, char** argv)
{
 ...
 printf("\n\nExample: imrev pic.bmppic_v.bmp v\n\n");
 return 0;
 }
 ...
 switch (argv[3][0]){
 case 'v' :
 for(k=0; k<RNDS; k++) imdata = RevImgV(imdata);
 break;
 case 'h' :
 for(k=0; k<RNDS; k++) imdata = RevImgH(imdata);
 break;
 default : printf("\nInvalid Option!\n"); 
return 0;
 }
 stop = clock();
 elaptime = 1000*((double)(stop-start))
 /(double)CLOCKS_PER_SEC/(double)RNDS;
 //寫入影像檔
 bmp_write(imdata, argv[2]);
 //釋放記憶體
 for(int i = 0; i < inf.vpxs; i++) { free(imdata[i]); }
 free(imdata);
