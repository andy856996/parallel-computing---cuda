...
// 垂直翻轉影像的Vrev kernel函式, 每個thread只負責翻轉單一的(R, G, B) 像素
__global__ void Vrev(uch * dstimg, uch * srcimg, ui hpxs, ui vpxs) {
  ...
  if (col >= hpxs) return; // col out of range
...
__global__ void Pixdup(uch * dstimg, uch * srcimg, ui FS) {
  ...
  ui tid = threadIdx.x;
  ui gtid = ThrPerBlk * bid + tid;
  if (gtid > FS) return; // outside the allocated memory
...
uch * bmp_read_1D(char * fn) {
  ...
  printf("\n Input File name: %17s (%u x %u) File Size=%u", fn, inf.hpxs, inf.vpxs, IMAGESIZE);
...
int main(int argc, char ** argv) {
  ...
  printf("--------------------------------------------------------------------------\n");
  printf("%s %s %s %c %u [%u BLOCKS, %u BLOCKS/ROW]\n", ProgName, InputFileName, OutputFileName, Rev, ThrPerBlk, NumBlocks, BlkPerRow);
  printf("--------------------------------------------------------------------------\n");
  printf("CPU->GPU Transfer =%7.2f ms ... %4d MB ... %6.2f GB/s\n", tfrCPUtoGPU, DATAMB(IMAGESIZE), DATABW(IMAGESIZE, tfrCPUtoGPU));
  printf("Kernel Execution = % 7.2 f ms... % 4 d MB... % 6.2 f GB / s\ n", kernelExecutionTime, DATAMB(GPUDataTransfer), DATABW(GPUDataTransfer, kernelExecutionTime));
